    ...
    <div className="col-lg-4 col-md-4 col-sm-6 mb-3 pe-2 ps-2">
      <div className="card w-100 p-0 hover-card shadow-xss border-0 rounded-3 overflow-hidden me-1">
        ...
        <div className="card-image w-100 mb-3">
          <Link to={`/productos/${product._id}`} className="position-relative d-block">
            <img
              loading="lazy"
              src={product?.Images?.length ? product?.Images[0] : "https://picsum.photos/385/300"}
              alt={product.titlePost}
              className="w-100"
            />
          </Link>
        </div>
        <div className="card-body pt-0">
          ...
          <h6 className="font-xsssss text-grey-500 fw-600 mt-0 mb-1"> {product?.descriptionPost}</h6>
          <div className="star d-block w-100 text-left mt-0">
            <i className="fa-solid fa-star text-bg-danger"></i>
            <i className="fa-solid fa-star-half"></i>
            {calcularPorcentaje(product.claps, product.no_claps) ?? ""}
          </div>
          ...
          <span className="font-lg fw-700 mt-0 pe-3 ls-2 lh-32 d-inline-block text-success float-left">
            <span className="font-xs">$</span> {product.price}
          </span>
          <Link to={`/productos/${product._id}`} className="position-absolute bottom-15 mb-2 right-15">
